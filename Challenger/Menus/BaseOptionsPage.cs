...
internal abstract class BaseOptionsPage : IClickableMenu {
    ...
    private readonly List<IOptionElement> _options = new(ItemsPerPage); // OptionsElement
    ...
    public BaseOptionsPage()
        : this(Game1.viewport.Width / 2 - (800 + borderWidth * 2) / 2,
            Game1.viewport.Height / 2 - (600 + borderWidth * 2) / 2,
            800 + borderWidth * 2, 600 + borderWidth * 2) {
    }

    public BaseOptionsPage(int x, int y, int width, int height)
    ...
    public void AddOption(OptionsElement option) {
        AddOption(new SOptionElement(InitSingleLineElement(option)));
    }
    
    protected TElement InitSingleLineElement<TElement>(TElement option) where TElement: OptionsElement {
        option.bounds = new Rectangle(
            option.bounds.X,
            option.bounds.Y,
            width - 3 * option.bounds.X,
            option.bounds.Height);
        return option;
    }

    public void AddOption(IOptionElement option) {
        _options.Add(option);
    }
    ...
    private bool HasActiveOverlay() {
        return _heldOption != null && _heldOption.Element.HasActiveOverlay();
    }
...
internal record HeldOption(ClickableComponent Slot, IOptionElement Element);